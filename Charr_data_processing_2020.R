...
###########################
# Lizy Mittell -- https://github.com/eamittell
# 3rd July 2020 -- final update
# 13th October 2020 -- updated for use with the 2020 field data, which has some differences in formatting

library(readxl)
library(tidyverse)

# Using the most recent data (2020) -- Make sure that the data is ordered with August 2012 at the top, becoming more recent thereafter.
data_r <- read_excel("Masterfile_Vertical_17_09_20.xls", sheet = "final")
data_o <- read_excel("Masterfile_Vertical_19_09_19.xls", sheet = "final") ## 2019 Masterfile
# Check and rename 
colnames(data_r)
names(data_r)[names(data_r) == "Date of capture"] <- "date"
names(data_r)[names(data_r) == "length"] <- "fl"
#names(data_r)[names(data_r) == "Location"] <- "cave" # For use with the 2019 Masterfile
names(data_r)[names(data_r) == "tag"] <- "id"

str(data_r)
data_r <- data_r[,-c(11:13)]
summary(data_r)

unique(data_r$date)
# August 2018 has 2022 instead -- correct
#data_r$date <- gsub("2022", "2018", data_r$date) ## this is in the 2019 data
...
# 2020 data also has "unknown" cave. Remove this as well
...
data_r <- subset(data_r[which(data_r$cave!="unknown"),])
...
# Not present in 2020 data, use with 2019
...
## 13/10/2020 -- 10,455 measurements
...
## 13/10/2020 -- 99 deaths from 87 indiv
...
# 13/10/2020 -- 10,036 measurements, 3898 ids
########################################### 
...
dim(dead_2) # 379 obs from 164 unique tags 3/7/2020
# 389 from 164 tags 13/10/2020
...
# 79 of them are unique 3/7/2020
# 80 13/10/2020
length(unique(z))
...
final_dead <- final_dead[,-14] # remove id_dead
...
# 10,181 observations from 4093 fish
